...
def plot_features(data_train, labels_train, name, defPath="", bin=90):
    ...
    for i in range(data_train.shape[0]):
        ...
        plt.legend(loc="best")
        plt.savefig(
            defPath + "data/Featuress/%s_%data.jpg" % (name, i),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close(fig)
...
def plot_correlations(data_train, labels_train, defPath=""):
    ...
    labels = {
        ...
        1: "Female (absolute Pearson coeff.)",
        2: "Male (absolute Pearson coeff.)",
    }

    CorrCoeff = {
        0: numpy.abs(numpy.corrcoef(data_train)),
        1: numpy.abs(numpy.corrcoef(data_train[:, labels_train == 1])),
        2: numpy.abs(numpy.corrcoef(data_train[:, labels_train == 0])),
    }
    for i in range(3):
        fig = plt.figure()
        plt.title(labels[i])
        ...
        plt.savefig(
            defPath + "data/Heatmaps/heatmap_%data.jpg" % i, dpi=300, bbox_inches="tight"
        )
        plt.close(fig)
...
def plot_minDCF_lr(labels, y5, y1, y9, filename, title, defPath=""):
    ...
    plt.legend(loc="best")
    plt.savefig(
        defPath + "data/LRresults/lr_minDCF_%s.jpg" % filename, dpi=300, bbox_inches="tight"
    )
    plt.close(fig)
...
def plot_minDCF_svm(C, y5, y1, y9, filename, title, type="linear", defPath=""):
    ...
    plt.legend(loc="best")
    plt.savefig(
        defPath + "data/SVMres/svm_minDCF_%s.jpg" % filename,
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
...
def plot_minDCF_gmm(components, y5, y1, y9, filename, title, defPath=""):
    ...
    plt.legend(loc="best")
    plt.savefig(
        defPath + "data/GMMres/gmm_minDCF_%s.jpg" % filename,
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
